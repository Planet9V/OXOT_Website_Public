...
export const PsychoHistorySphere: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Process Entities
      const projected = entities.map(p => {
      ...
      // Optimized connection drawing: only draw if "Symbolic" and close to center of Z
      // (Skipping full mesh for performance, just drawing rings implies structure)
      ...
      projected.forEach(p => {
        ...
        if (p.layer === 'REAL') {
          ...
          const size = 1 * p.scale;
          ctx.beginPath();
          ctx.arc(p.x, p.y, size, 0, Math.PI * 2);
          ctx.fill();
        } else if (p.layer === 'CALCULUS') {
          ctx.font = `${12 * p.scale}px 'Geist Mono', monospace`;
      ...
      const grd = ctx.createRadialGradient(cx, cy, 0, cx, cy, width * 0.15 * corePulse);
