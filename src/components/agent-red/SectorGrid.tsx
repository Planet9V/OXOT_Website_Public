    ...
    return (
        <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
                {sectors.map((sector) => (
                    <div key={sector.id} className="group relative bg-black/60 border border-white/10 p-4 h-48 flex flex-col transition-all duration-300 hover:border-red-500/50 hover:bg-red-900/10 hover:shadow-[0_0_20px_rgba(239,68,68,0.2)]">
                        {/* Target Reticle Corners */}
                        <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-white/20 group-hover:border-red-500 transition-colors"></div>
                        <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-white/20 group-hover:border-red-500 transition-colors"></div>
                        <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-white/20 group-hover:border-red-500 transition-colors"></div>
                        <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-white/20 group-hover:border-red-500 transition-colors"></div>

                        <div className="flex justify-between items-start mb-4">
                            <div className="text-gray-500 group-hover:text-red-400 transition-colors">
                                {sector.icon}
                            </div>
                            <span className="font-mono text-[10px] text-gray-600 group-hover:text-red-500">ID: {sector.id.toString().padStart(2, '0')}</span>
                        </div>

                        <h4 className="font-bold text-white text-sm uppercase mb-2 tracking-tight group-hover:text-red-100">{sector.name}</h4>
                        <p className="text-[10px] text-gray-400 leading-relaxed line-clamp-3 group-hover:text-gray-300">
                            {sector.details}
                        </p>

                        {/* Lock State */}
                        <div className="mt-auto pt-4 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="text-[9px] font-mono text-red-500 animate-pulse">● TARGET LOCKED</span>
