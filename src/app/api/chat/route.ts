    ...
    try {
        ...
        try {
            const qdrantRes = await fetch(`${QDRANT_URL}/collections/ner11_entities_hierarchical/points/search`, {
                ...
                body: JSON.stringify({
                    vector: [/* Needs embedding logic or just keyword search if vector is not ready */],
                    limit: 3,
                    with_payload: true
                })
            });
        ...
        // 2. Synthesize AI Response via Perplexity
        ...
        try {
            const perpRes = await fetch('https://api.perplexity.ai/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: 'system', content: 'You are the AEON Psychometric Calculus Engine. Respond as a sophisticated, analytical AI. Keep it concise.' },
                        ...history,
